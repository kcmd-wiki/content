...
반드시 커맨드블록을 이용해야 한다는 단점이 존재합니다.
인첸트 버그의 특성상 반드시 커맨드블록의 출력에 접근해야 하기에 어쩔 수 없습니다.

큰따옴표(`"`) 기호가 들어가면 고장나며,
역슬래시(`\`) 기호가 들어가면 역슬래시 두개로 불어납니다.
커맨드블록의 출력이 JSON 텍스트이기에 이 두 기호는 아주 불안정합니다.
반대로 작은따옴표 기호는(`'`) 아무런 부작용 없이 멀쩡합니다. 가능하다면 이것을 이용하세요.
...
인첸트 방법과 달리 커맨드블록이 필요없기에 함수에서도 활용이 가능합니다.
...
이중에서 큰따옴표(`"`), 역슬래시(`\`), 작은따옴표(`'`) 기호를 제외하면은 고장을 피할수 있는 방법이 존재합니다.
...
인첸트 버그와 독서대 기술을 적절히 활용하면 동적으로 JSON 텍스트를 구성하는게 가능합니다.

예를 들어, 출력할 텍스트 색상을 스코어보드값으로 결정할 수 있다거나,
...
위에서 설명한 "인첸트"와 "독서대"를 알고 있다는 전제 하에 진행합니다.
